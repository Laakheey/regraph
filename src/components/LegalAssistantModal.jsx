...
export const LegalAssistantModal = ({ isOpen, onClose }) => {
  ...
  const handleAsk = (promptText) => {
    ...
    } else if (qLower.includes('trade secret') || qLower.includes('vendor') || qLower.includes('sas')) {
    ...
    } else if (qLower.includes('gap') || qLower.includes('growth plan') || qLower.includes('santos')) {
    ...
    } else if (qLower.includes('opinion') || qLower.includes('gilmore') || qLower.includes('summary judgment')) {
  ...
  return (
    <div className="modal-backdrop centered" onClick={onClose}>
      <div className="modal-dialog assistant-modal" onClick={(e) => e.stopPropagation()}>
            ...
            {messages.map((m, idx) => (
              <div key={idx} className={`chat-bubble ${m.sender === 'user' ? 'user-bubble' : 'assistant-bubble'}`}>
                <div className="bubble-author">
                  {m.sender === 'user' ? 'Investigator' : 'TRACE Legal Assistant'}
                </div>
          ...
          <div className="canned-prompts-tray">
            ...
            <div className="prompts-list">
              {cannedPrompts.map((cp, idx) => (
                <button key={idx} className="canned-chip" onClick={() => handleAsk(cp)}>
                  {cp}
                </button>
              ))}
            </div>
